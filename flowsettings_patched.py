...
KH_INDEX_TYPES = [
    ...
    *GRAPHRAG_INDEX_TYPES,
    # RAGFlow-backed index (best accuracy, deepdoc parsing)
    *([
        "ragflow_integration.RAGFlowIndex"
    ] if _RAGFLOW_AVAILABLE else []),
]
...
KH_INDICES = [
    ...
    *GRAPHRAG_INDICES,
    # RAGFlow knowledge base — deepdoc parsing + hybrid retrieval
    *([{
        "name": "RAGFlow Collection",
        "config": {
            "supported_file_types": (
                ".png, .jpeg, .jpg, .tiff, .tif, .pdf, .xls, .xlsx, .doc, .docx, "
                ".pptx, .csv, .html, .mhtml, .txt, .md, .zip"
        ...
        "index_type": "ragflow_integration.RAGFlowIndex",
    }] if _RAGFLOW_AVAILABLE else []),
]
